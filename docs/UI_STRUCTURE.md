...
- **✨ AI Auto-Edit button** (header, `ai-auto-edit-button.tsx`) — the
  **deterministic pipeline auto-edit**. On click:
  `remoteClips.requestAiEdit({ clipId, { chipAction: "auto" } })` →
  `remoteClips.applyEdl({ edl })` (one undoable `ApplyEdlCommand`). It lays down
  the pipeline's reaction **zooms** (scale keyframes on the video), **onomatopoeia
  burst captions** (an "AI Zoom Captions" text track), and **SFX** (an "AI SFX"
  audio track, from `public/sfx/*.wav`). Effects source: the run's
  `out/<stream>/edl/clip_NN.edl.json` (produced by `pipeline/autoedit.py`).
